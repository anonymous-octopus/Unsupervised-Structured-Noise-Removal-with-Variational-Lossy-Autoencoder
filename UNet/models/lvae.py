...
class UNet(LightningModule):
    ...
    def forward(self, x, y):
        ...
        # Calculate mean squared error between signal estimate and y
        if not self.mode_pred:
            predicted_s, mse = self.s_decoder.get_s(predicted_s_code, y)
            mse = mse.mean()
        else:
            predicted_s, _ = self.s_decoder.get_s(predicted_s_code, torch.zeros_like(x))
            mse = None
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            all_samples = self.forward(torch.repeat_interleave(x[idx:idx + 1], 10, 0), torch.repeat_interleave(y[idx:idx + 1], 10, 0))['predicted_s']
    ...
    def predict_step(self, batch, _):
        ...
        out = self.forward(x, None)['predicted_s']

...
class NormalStochasticBlock2d(nn.Module):
    ...
    def __init__(self, c_in, c_vars, c_out, kernel=3, transform_p_params=True):
        ...
        if transform_p_params:
            self.conv_in_p = nn.Conv2d(c_in, 2 * c_vars, kernel, padding=pad, padding_mode='replicate')
        self.conv_in_q = nn.Conv2d(c_in, 2 * c_vars, kernel, padding=pad, padding_mode='replicate')
        self.conv_out = nn.Conv2d(c_vars, c_out, kernel, padding=pad, padding_mode='replicate')
    ...
    def forward(self,
        ...
        # p = Normal(p_mu, p_lstd.exp())

        if q_params is not None:
            # Define q(z)
            q_params = self.conv_in_q(q_params)
            q_mu, q_lstd = q_params.chunk(2, dim=1)
            # q = Normal(q_mu, q_lstd.exp())

            # Sample from q(z)
            sampling_distrib_mean = q_mu
        else:
            # Sample from p(z)
            sampling_distrib_mean = p_mu

        # Generate latent variable (typically by sampling)
        if forced_latent is None:
            if use_mode:
                z = sampling_distrib_mean
            else:
                if mode_pred:
